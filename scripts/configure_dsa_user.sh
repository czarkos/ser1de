...
# Script configure DSA devices
# Usage : ./configure_dsa_user <mode> <start,end> <wq_size> <engine_number>
...
# wq_size: 1-128
# engine_number: 1-4 - number of engines per device
...
max_wqs=8
# Max WQ size -- CHANGE IF REQUIRED.
max_wq_size=128
...
assert "[ $max_wq_size -ge $3 ]" "Assertion failed: Max Work Queue Size is 128"
assert "[ $max_engines -ge $4 ]" "Assertion failed: 4 Max Engines per device"

#
# Count IAA instances in the system.
#
dsa_dev_id="0b25"
num_dsa=$(lspci -d:${dsa_dev_id} | wc -l)
echo "Found ${num_dsa} DSA instances"

#
# Parse input.
#
dedicated=${1:-0}; shift
device_num=${1:-$num_dsa}; shift
wq_size=${1:-128}; shift
...
#
# Disable all DSA devices, engines, and WQs.
#
echo "Disable all DSA devices, engines, and WQs."

first=0 && step=2
for ((i = ${first}; i < ${step} * ${num_dsa}; i += ${step})); do
 for ((j = 0; j < ${max_wqs}; j += 1)); do
  accel-config disable-wq dsa${i}/wq${i}.${j}
 done

 echo disable dsa dsa${i}
...
# Enable loop.
echo "Enable DSA devices: ${start} to ${end}"
for ((i = ${start}; i < ${end}; i += ${step})); do
 # Config engines.
 for ((j = 0; j < ${engine_number}; j += 1)); do
  echo enable engine dsa${i}/engine${i}.${j}
  accel-config config-engine dsa${i}/engine${i}.${j} --group-id=0
 done
 ...
 for ((j = 0; j < ${max_wqs}; j += 1)); do
  ...
  # 
  # Basic config.
  #accel-config config-wq dsa${i}/wq${i}.${j} -g 0 -s $wq_size -p 10 -m ${mode} -y user -n user${i} -t $wq_size -d user
