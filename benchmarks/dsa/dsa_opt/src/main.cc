...
static constexpr size_t kNofIterations = 3001;
...
int main () {
    ...
    std::chrono::steady_clock::time_point gather_schema_begin, gather_schema_end;
    std::chrono::nanoseconds gather_schema_duration;
    gather_schema_begin = std::chrono::steady_clock::now();
    for (size_t i = 0; i < kNofIterations; ++i) {
    ...
    gather_schema_end = std::chrono::steady_clock::now();
    gather_schema_duration = std::chrono::duration_cast<std::chrono::nanoseconds>(gather_schema_end - gather_schema_begin);
    std::cout << "average gather schema time = " << (gather_schema_duration.count() / kNofIterations) << " ns\n";
    ...
    for (size_t i = 0; i < kNofIterations; ++i) {
        // std::cout << "I AM HEREEEEEEE" << i << std::endl;
         ...
         auto outcome = scagatherer.GatherWithMemCpy(gather_schemas[i], gather_outs[i].data(), &out_size);
         //scagatherer.dsa_gather_blocking(gather_schemas[i], gather_outs[i].data(), &out_size);
         end = std::chrono::steady_clock::now();
