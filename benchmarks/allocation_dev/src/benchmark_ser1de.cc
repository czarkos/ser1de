...
inline void benchmark_ser1de_serialize(ScatterGather& scagatherer, IAAComp& iaa_comp, std::vector<M>& messages, std::vector<std::unique_ptr<uint8_t[]>>& compressed, std::vector<uint32_t>& comprOutputSize, std::vector<std::chrono::nanoseconds>& gather_schema_durations, std::vector<std::chrono::nanoseconds>& gather_durations, std::vector<std::chrono::nanoseconds>& compression_durations, std::vector<std::vector<size_t>>& sizes_for_scatter) {
    ...
    for (size_t i = 0; i < kNofIterations; ++i) {
        ...
        // <------------ COMPRESS ------>
        begin = std::chrono::steady_clock::now();
        iaa_comp.compress_blocking(gather_out.data(), gather_out.size(), compressed[i].get(), BUFFER_SIZE, &comprOutputSize[i]);
...
inline void benchmark_ser1de_deserialize(ScatterGather& scagatherer, IAAComp& iaa_comp, std::vector<M>& out_messages, std::vector<std::unique_ptr<uint8_t[]>>& compressed, std::vector<uint32_t>& comprOutputSize, std::vector<uint32_t>& decomprOutputSize, std::vector<std::chrono::nanoseconds>& decompression_durations, std::vector<std::chrono::nanoseconds>& scatter_schema_durations, std::vector<std::chrono::nanoseconds>& scatter_durations) {
    std::chrono::steady_clock::time_point begin, end;
    std::chrono::nanoseconds duration;
...
int benchmark (bool dsa) {
    ...
    */
    ...
    benchmark_ser1de_deserialize(scagatherer, iaa_comp, out_messages, compressed, comprOutputSize, decomprOutputSize, decompression_durations, scatter_schema_durations, scatter_durations);
