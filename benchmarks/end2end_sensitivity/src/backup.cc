...
inline void benchmark_decompress(IAAComp& iaa_comp, std::vector<std::unique_ptr<uint8_t[]>>& compressed, std::vector<uint32_t>& comprOutputSize, std::vector<std::vector<uint8_t>>& gather_outs, std::vector<uint32_t>& decomprOutputSize, std::vector<std::chrono::nanoseconds>& decompression_durations) {
    ...
    for (size_t i = 0; i < kNofIterations; ++i) {
        begin = std::chrono::steady_clock::now();
        iaa_comp.decompress_blocking(compressed[i].get(), comprOutputSize[i], gather_outs[i].data(), BUFFER_SIZE, &decomprOutputSize[i]);
        //iaa_comp.decompress_blocking(compressed[i].get(), comprOutputSize[i], gather_outs[0].data(), BUFFER_SIZE, &decomprOutputSize[i]);
        end = std::chrono::steady_clock::now();
...
int benchmark (bool dsa) {
    ...
    */
    ...
    //
    std::vector<ScatterGather::Schema> gather_schemas(kNofIterations);
    std::vector<std::vector<size_t>> sizes_for_scatter(kNofIterations);
    ...
    //
    std::vector<ScatterGather::Schema> scatter_schemas(kNofIterations);
