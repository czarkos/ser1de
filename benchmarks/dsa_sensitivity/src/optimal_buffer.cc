...
int main () {
    ...
    // IAA utility arrays
    uint8_t** compressed   = new uint8_t*[LOG_MAX_BUFFER_SIZE];
    uint8_t** decompressed   = new uint8_t*[LOG_MAX_BUFFER_SIZE];
    // arrays for keeping output size feedback from IAA
    uint32_t* comprOutputSize = new uint32_t[LOG_MAX_BUFFER_SIZE];
    uint32_t* decomprOutputSize = new uint32_t[LOG_MAX_BUFFER_SIZE];

    for (size_t i = 0; i < LOG_MAX_BUFFER_SIZE; ++i) {
        size_t BFR_SIZE = 1 << i;
        compressed[i] = new uint8_t[BFR_SIZE];
        decompressed[i] = new uint8_t[BFR_SIZE];
        comprOutputSize[i] = 0;
        decomprOutputSize[i] = 0;
    }
    ...
    std::vector<uint8_t> gather_outs(BUFFER_SIZE, 0);
    ...
    for (size_t i = 0; i < kNofIterations; ++i) {
        ...
        size_t index = ceil(log2(out_size));
        assert(index >= 0);
        assert(index <= LOG_MAX_BUFFER_SIZE);
        outcome = compress_with_IAA(gather_outs.data(), out_size, compressed[index], BUFFER_SIZE, &comprOutputSize[index]);
        end = std::chrono::steady_clock::now();
    ...
    for (size_t i = 0; i < LOG_MAX_BUFFER_SIZE; ++i) {
        std::cout << "compress_out(bytes)[" << i << "], " << comprOutputSize[i] << "\n";
    }
    ...
    for (size_t i = 0; i < messages.size(); ++i) {
        ...
        assert(index >= 0);
        assert(index <= LOG_MAX_BUFFER_SIZE);
        auto outcome = decompress_with_IAA(compressed[index], comprOutputSize[index], decompressed[index], BUFFER_SIZE, &decomprOutputSize[index]);
        end = std::chrono::steady_clock::now();
        ...
        outcome = scagatherer.ScatterWithMemCpy(decompressed[index], scatter_schemas[i]);
        end = std::chrono::steady_clock::now();
    ...
    for (size_t i = 0; i < LOG_MAX_BUFFER_SIZE; ++i) {
        std::cout << "decompress_out(bytes)[" << i << "], " << decomprOutputSize[i] << "\n";
    }
    /*
