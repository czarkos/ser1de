...
inline void benchmark_compress(IAAComp& iaa_comp, std::vector<std::vector<uint8_t>>& gather_outs, std::vector<uint8_t*>& compressed, std::vector<uint32_t>& comprOutputSize, std::vector<std::chrono::nanoseconds>& compression_durations) {
    std::chrono::steady_clock::time_point begin, end;
    std::chrono::nanoseconds duration;

    for (size_t i = 0; i < kNofIterations; ++i) {
        begin = std::chrono::steady_clock::now();
        iaa_comp.compress_blocking(gather_outs[i].data(), gather_outs[i].size(), compressed[i], BUFFER_SIZE, &comprOutputSize[i]);
        end = std::chrono::steady_clock::now();
...
inline void benchmark_decompress(IAAComp& iaa_comp, std::vector<uint8_t*>& compressed, std::vector<uint32_t>& comprOutputSize, std::vector<std::vector<uint8_t>>& gather_outs, std::vector<uint32_t>& decomprOutputSize, std::vector<std::chrono::nanoseconds>& decompression_durations) {
    std::chrono::steady_clock::time_point begin, end;
    std::chrono::nanoseconds duration;

    for (size_t i = 0; i < kNofIterations; ++i) {
        begin = std::chrono::steady_clock::now();
        iaa_comp.decompress_blocking(compressed[i], comprOutputSize[i], gather_outs[i].data(), BUFFER_SIZE, &decomprOutputSize[i]);
        end = std::chrono::steady_clock::now();
