...
inline size_t benchmark_ser1de_serialize(Ser1de_re &ser1de, std::vector<google::protobuf::Message*>& messages, std::vector<std::string>& ser_outs, size_t num_requests, std::vector<std::chrono::nanoseconds>& latencies) {
    // Warmup iterations
    for (size_t i = 0; i < kNofWarmupIterations; i++) {
...
inline void benchmark_ser1de_deserialize(Ser1de_re &ser1de, std::vector<std::string>& ser_outs, std::vector<google::protobuf::Message*>& deser_messages_out, size_t num_requests, std::vector<std::chrono::nanoseconds>& latencies) {
    // Warmup iterations
    for (size_t i = 0; i < kNofWarmupIterations; i++) {
...
int benchmark (size_t num_requests, Ser1de_re& ser1de) {
...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " [num_threads] [num_requests]" << std::endl;
    std::cout << "  num_threads: Number of threads to use (default: 2)" << std::endl;
    std::cout << "  num_requests: Number of requests per thread (default: 30000)" << std::endl;
    std::cout << "Example: " << program_name << " 4 50000" << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        char* endptr;
        num_threads = strtoul(argv[1], &endptr, 10);
        if (*endptr != '\0' || num_threads == 0) {
            std::cerr << "Error: Invalid number of threads: " << argv[1] << std::endl;
            print_usage(argv[0]);
            return 1;
        }
    }
    ...
    if (argc > 2) {
        char* endptr;
        num_requests = strtoul(argv[2], &endptr, 10);
        if (*endptr != '\0' || num_requests == 0) {
            std::cerr << "Error: Invalid number of requests: " << argv[2] << std::endl;
            print_usage(argv[0]);
            return 1;
        }
    }
    ...
    std::cout << "Requests per thread: " << num_requests << std::endl;
    std::cout << "Total requests: " << (num_threads * num_requests) << std::endl;
    std::cout << "-----------------------------------------------------" << std::endl;
    ...
    for (size_t i = 0; i < num_threads; ++i) {
        threads.emplace_back([&shared_ser1de, num_requests, i]() {
            std::cout << "Thread " << i << " starting benchmark..." << std::endl;
            benchmark(num_requests, shared_ser1de);
            std::cout << "Thread " << i << " finished benchmark" << std::endl;
        });
    }
    ...
    std::cerr << "All " << num_threads << " threads finished" << std::endl;
}