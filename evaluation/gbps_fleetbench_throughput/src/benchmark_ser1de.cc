...
inline double benchmark_serialize(std::vector<google::protobuf::Message*>& messages, std::vector<std::string>& ser_outs, size_t num_requests) {
    ...
    for (size_t i = 0; i < num_requests; i++) {
        total_bytes += messages[i]->SpaceUsedLong();
    }
    ...
    return (total_bytes * 8.0) / (elapsed_s * 1e9);
}

inline double benchmark_deserialize(std::vector<google::protobuf::Message*>& messages, std::vector<std::string>& ser_outs, std::vector<google::protobuf::Message*>& deser_messages_out, size_t num_requests) {
    for (size_t i = 0; i < kNofWarmupIterations; i++) {
...
inline double benchmark_ser1de_serialize(Ser1de &ser1de, std::vector<std::string>& ser_outs, std::vector<google::protobuf::Message*>& messages, size_t num_requests) {
    for (size_t i = 0; i < kNofWarmupIterations; i++) {
...
inline double benchmark_ser1de_deserialize(Ser1de &ser1de, std::vector<google::protobuf::Message*>& messages, std::vector<std::string>& ser_outs, std::vector<google::protobuf::Message*>& deser_messages_out, size_t num_requests) {
    for (size_t i = 0; i < kNofWarmupIterations; i++) {
...
int benchmark(size_t num_requests, BenchmarkType type) {
    ...
    switch (type) {
        case PROTO_SERIALIZE:
            gbps = benchmark_serialize(messages, proto_ser_outs, num_requests);
            type_name = "proto_serialize";
            break;
        case PROTO_DESERIALIZE:
            benchmark_serialize(messages, proto_ser_outs, num_requests);
            gbps = benchmark_deserialize(messages, proto_ser_outs, proto_deser_messages_out, num_requests);
            type_name = "proto_deserialize";
            break;
        case SER1DE_SERIALIZE:
            gbps = benchmark_ser1de_serialize(ser1de, ser1de_ser_outs, messages, num_requests);
            type_name = "ser1de_serialize";
            break;
        case SER1DE_DESERIALIZE:
            benchmark_ser1de_serialize(ser1de, ser1de_ser_outs, messages, num_requests);
            gbps = benchmark_ser1de_deserialize(ser1de, messages, ser1de_ser_outs, ser1de_deser_messages_out, num_requests);
            type_name = "ser1de_deserialize";
            break;
    }

    std::cout << type_name << "," << num_requests << "," << std::fixed << std::setprecision(4) << gbps << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    static constexpr size_t kNumMessages = 100000;

    std::cout << "type,num_messages,gbps" << std::endl;

    for (auto type : types) {
        for (size_t trial = 0; trial < num_trials; trial++) {
            benchmark(kNumMessages, type);
        }
    }
    return 0;
}