...
inline double benchmark_serialize(std::vector<google::protobuf::Message*>& messages, std::vector<std::string>& ser_outs, size_t num_requests, size_t total_bytes) {
...
inline double benchmark_deserialize(std::vector<std::string>& ser_outs, std::vector<google::protobuf::Message*>& deser_messages_out, size_t num_requests, size_t total_bytes) {
    for (size_t i = 0; i < kNofWarmupIterations; i++) {
...
inline std::tuple<double, size_t> benchmark_ser1de_serialize(Ser1de &ser1de, std::vector<std::string>& ser_outs, std::vector<google::protobuf::Message*>& messages, size_t num_requests) {
    for (size_t i = 0; i < kNofWarmupIterations; i++) {
    ...
    for (size_t i = 0; i < num_requests; i++) {
        ...
        total_bytes += ser1de.get_ser_gather_out_size();
    }
    auto end = std::chrono::steady_clock::now();

    double elapsed_s = std::chrono::duration<double>(end - start).count();
    return { (total_bytes * 8.0) / (elapsed_s * 1e9), total_bytes };
}

inline double benchmark_ser1de_deserialize(Ser1de &ser1de, std::vector<std::string>& ser_outs, std::vector<google::protobuf::Message*>& deser_messages_out, size_t num_requests, size_t total_bytes) {
    for (size_t i = 0; i < kNofWarmupIterations; i++) {
...
int benchmark(size_t num_requests) {
    ...
    std::cout << "proto_serialize" << "," << num_requests << "," << std::fixed << std::setprecision(4) << proto_ser_gbps << std::endl;

    double proto_deser_gbps = benchmark_deserialize(proto_ser_outs, proto_deser_messages_out, num_requests, total_bytes);
    std::cout << "proto_deserialize" << "," << num_requests << "," << std::fixed << std::setprecision(4) << proto_deser_gbps << std::endl;

    double ser1de_deser_gbps = benchmark_ser1de_deserialize(ser1de, ser1de_ser_outs, ser1de_deser_messages_out, num_requests, total_bytes);
    std::cout << "ser1de_deserialize" << "," << num_requests << "," << std::fixed << std::setprecision(4) << ser1de_deser_gbps << std::endl;
